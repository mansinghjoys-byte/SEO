...
### Frontend (.env)
```bash
REACT_APP_BACKEND_URL=https://rankforge.preview.emergentagent.com
```
...
```bash
curl -X POST https://rankforge.preview.emergentagent.com/api/auth/register \
...
### Test Login
```bash
curl -X POST https://rankforge.preview.emergentagent.com/api/auth/login \
  -H "Content-Type: application/json" \
...
- API docs: `https://rankforge.preview.emergentagent.com/api/docs`
